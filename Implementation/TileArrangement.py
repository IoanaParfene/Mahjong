...
def arrange_tiles(version):
    ''' Create the selected shape of the piece model'''
    if(version == 'standard'):
...
def render_pieces(screen):
    ...
    for level in range(0,5):
        for line in range(0,9):
            for column in range(0, 16):
                if tile_array[level][line][column][0] == True:
...
def init_piece_arrangement(gameVersion):
    ''' Initialize the pieces and the arrangement '''
    load_piece_images()
    create_tile_array()
    arrange_tiles(gameVersion)
